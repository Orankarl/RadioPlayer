...
public class SingleLrcView extends View {
    ...
    public SingleLrcView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SingleLrcView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        float v = (currentMillis - start) > 500 ? currentPosition * lineHeight : lastPosition * lineHeight + (currentPosition - lastPosition) * lineHeight * ((currentMillis - start) / 500f);
        float v = (currentMillis - start) > 500 ? currentPosition * lineHeight : (currentPosition - 1) * lineHeight + lineHeight * ((currentMillis - start) / 500f);
        setScrollY((int) (v - 0.1*lineHeight));

        if (getScrollY() == currentPosition * lineHeight - 0.1 * lineHeight) {
...
//            e.printStackTrace();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        setMeasuredDimension(widthMeasureSpec, (int)(lineHeight * 1.25));
    }
}
